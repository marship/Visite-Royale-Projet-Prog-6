...
public class ListePlateaux {
    ...
    private Sequence<CoupleAtteindrePlateau> calculGardes(Sequence<CoupleAtteindrePlateau> res, int[] positions, int[] cartesJouees) {
        // Jouer les cartes des gardes
        res = calculDeplacementGarde(res, positions, cartesJouees);
        return res;
    }

    private Sequence<CoupleAtteindrePlateau> calculRoi(Sequence<CoupleAtteindrePlateau> res, int[] positions, int[] cartesJouees) {
    ...
    private Sequence<CoupleAtteindrePlateau> calculDeplaceRoi(Sequence<CoupleAtteindrePlateau> res, int[] base, int[] cartes, int nbRoi, int nbJouee) {
        remettreLesPositions(base);
        int tmpR = base[0];
        int tmpGG = base[1];
        int tmpGD = base[2];
        ...
        if(nbRoi >= 2){
            if(jeu.obtenirPositionElement(Element.GARDE_GAUCHE) != -8 || jeu.obtenirPositionElement(Element.GARDE_DROIT) != 8){
    ...
    private Sequence<CoupleAtteindrePlateau> calculDeplacementGarde(Sequence<CoupleAtteindrePlateau> res, int[] base, int[] cartes) {
        int i = 0;
        int tmpGG = base[1];
        int tmpGD = base[2];
        while(i < 8){
                ...
                while(j < 17){
                    if(deplacementPossibles[j] == 1){
                        if(carte.deplacement() == Deplacement.RAPPROCHE){
                            ...
                            CoupleAtteindrePlateau couple = new CoupleAtteindrePlateau(base, cartes);
                            res.insereQueue(couple);
                            calculDeplacementGarde(res, base, cartes);
                            base[1] = tmpGG;
                            base[2] = tmpGD;
                            cartes[i] = 0;
                        }
